...
#    name.Z <- names(Z)
    ...
    colnames(Z) <- nomiZ.vett <- rep(nome, times = a) #SERVE??? sì perché Z è senza colnames
...
#    beta.c<-if(k == 1) coef(obj)["U"] else coef(obj)[paste("U", 1:ncol(U), sep = "")]
    ...
    nomiU   <- unlist(mapply(forma.nomiU, length.psi, name.Z)) #in realtà non serve, c'era già!

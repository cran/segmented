...
if(isLeft) {
  ...
  #-----------
    ...
    if("try-error" %in% class(obj0))  stop("The first fit with U variables does not work..", call.=FALSE)#obj0 <- suppressWarnings(eval(call.noV.noinit)) ##a volte con i valori iniziali arima() )non converge!! Quindi provo senza init
    ...
    if (visual) {
      cat(paste("iter = ", sprintf("%2.0f",0),
                "  min.f = ", sprintf(paste("%", n.intDev0+6, ".5f",sep=""), L0), #formatC(L1,width=8, digits=5,format="f"), #era format="fg" 
                "  k = ", sprintf("%2.0f", NA),
...
#browser()
    while(abs(epsilon) > toll) {
      ...
      beta.c <- coef(obj)[nomiU] #coef(obj)[paste("U", 1:ncol(U), sep = "")]
      gamma.c <- coef(obj)[nomiV] #coef(obj)[paste("V", 1:ncol(V), sep = "")]
      ...
      #if(class(obj1)=="try-error")  obj1 <- suppressWarnings(eval(call.noV.noinit))
      L1<- if(class(obj1)[1]=="try-error") L0+10 else eval(parse(text=fn.obj), list(x=obj1))
      use.k<-k<-1
      L1.k<-NULL
      ...
      if (visual) {
        ...
        cat(paste("iter = ", sprintf("%2.0f",it),
                  "  min.f = ", sprintf(paste("%", n.intDev0+6, ".5f",sep=""), L1), #formatC(L1,width=8, digits=5,format="f"), #era format="fg" 
                  "  k = ", sprintf("%2.0f", k),
                  "  n.psi = ",formatC(length(unlist(psi)),digits=0,format="f"), 
    ...
    names(psi)<-id.psi.group
    names.coef<-names(coef(obj)) #obj e' quello vecchio che include U1,.. V1,...
    ...
    #which name? "coef" or "coefficients" or...
...
#    browser()
    ...
    obj<-list(obj=obj,it=it,psi=psi, psi.values=psi.values, U=U,V=V,rangeZ=rangeZ,
              epsilon=epsilon,nomiOK=nomiOK, SumSquares.no.gap=L1, id.psi.group=id.psi.group,id.warn=id.warn,
    ...
    #       k<-ncol(Z)
    #       #aggiorna la call, altrimenti il modello avra' sempre lo stesso numero di termini anche 
    #       # se alcuni psi vengono rimossi!!!

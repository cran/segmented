...
#    if(nrow(Z)!=length(y)) stop("length(Z) is different from the length of the response")
    ...
    w<-model.weights(objframe$model) #sarebbero i obj$prior.weights
    ...
    maxit.glm<-control$maxit.glm
        while(abs(epsilon)>toll){
            ...
            obj<-suppressWarnings(glm.fit(x=X,y=y,offset=o,weights=w,family=fam,
            ...
            if(it>it.max) break
            #se non-break estrai i risultati..
            if(k==1) {
                beta.c<-coef(obj)["U"]
                gamma.c<-coef(obj)["V"]} else {
